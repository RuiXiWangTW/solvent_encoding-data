...
class DescriptorGraphCollator(object):
    ...
    def collate(self, items):
        ...
        """
        smiles_batch_graph = batch_graphs([item[0][0] for item in items])
        solvent_batch_tensor = torch.cat([item[0][1] for item in items], dim = 0)
        target = torch.from_numpy(np.concatenate([item[1] for item in items]))
        return (smiles_batch_graph, solvent_batch_tensor), target


class DescriptorGraphDataLoader(torch.utils.data.DataLoader):
    ...
    **With Distributed Data Parallel**
    ...
    collator_arglist = inspect.getfullargspec(DescriptorGraphCollator).args
    ...
    def __init__(
        ...
        if collate_fn is None and kwargs.get("batch_size", 1) is not None:
            collate_fn = DescriptorGraphCollator(**collator_kwargs).collate
