...
class PharmHGT(nn.Module):
    def __init__(self,args):
        ...
        ## predict
        descriptor_length = 0
        if args["descriptor"] == "mn":
            descriptor_length = 7
        elif args["descriptor"] == "catalan":
            descriptor_length = 4
        elif args["descriptor"] == "reichardt":
            descriptor_length = 2
        self.out = nn.Sequential(nn.Linear(4*hid_dim + descriptor_length,hid_dim),
                                 self.act,
    ...
    def forward(self,bg):
        ...
        embed = torch.cat([embed_f,embed_aug, solvent],1)
        out = self.out(embed)
