...
class PharmHGT(nn.Module):
    def __init__(self,args):
        ...
        self.solvent_atom = nn.Linear(args['atom_dim'],hid_dim)
        self.solvent_bond = nn.Linear(args['bond_dim'],hid_dim)
        # pharm view
        self.solvent_pharm = nn.Linear(args['pharm_dim'],hid_dim)
        self.solvent_reac = nn.Linear(args['reac_dim'],hid_dim)
        # junction view
        self.solvent_junc = nn.Linear(args['atom_dim'] + args['pharm_dim'],hid_dim)

        ## define the view during massage passing
        self.solvent_mp = MVMP(msg_func=add_attn,hid_dim=hid_dim,depth=self.depth,view='a',suffix='h',act=self.act)
        self.solvent_mp_aug = MVMP(msg_func=add_attn,hid_dim=hid_dim,depth=self.depth,view='ap',suffix='aug',act=self.act)
        
        ## readout
        self.solvent_readout = Node_GRU(hid_dim)
        self.solvent_readout_attn = Node_GRU(hid_dim)

        ## predict
        self.out = nn.Sequential(nn.Linear(8*hid_dim,hid_dim),
                                 self.act,
    ...
    def forward(self,bg):
        ...
        # smiles initialize
        self.init_feature(smiles)
        self.mp(smiles)
        self.mp_aug(smiles)
        smiles_embed_f = self.readout(smiles,'h')
        smiles_embed_aug = self.readout_attn(smiles,'aug')
        embed_smiles = torch.cat([smiles_embed_f,smiles_embed_aug],1)
        #solvent inintialize
        self.init_feature(solvent)
        self.mp(solvent)
        self.mp_aug(solvent)
        solvent_embed_f = self.readout(solvent,'h')
        solvent_embed_aug = self.readout_attn(solvent,'aug')
        solvent_embed = torch.cat([solvent_embed_f,solvent_embed_aug],1)
        embed = torch.cat([embed_smiles, solvent_embed], dim = 1)
        out = self.out(embed)
        return out
