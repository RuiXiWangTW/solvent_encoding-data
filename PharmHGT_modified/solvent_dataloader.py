...
from data_with_descriptors import Mol2HeteroGraph
...
from dgl import merge as merge
from dgl import backend as F
from dgl._ffi.base import is_tensor_adaptor_enabled

from dgl.base import dgl_warning, DGLError, EID, NID
...
class SolventGraphCollator(object):
    ...
    def collate(self, items):
        ...
        """
        assist_graph = Mol2HeteroGraph(Chem.MolFromSmiles("CC(=O)N(C)C"))

        solvent_graphs = []
        smiles_graphs = []
        for item in items:
            smiles_graphs.append(item[0][0])
            solvent_graph = merge((item[0][1], assist_graph))
            solvent_graphs.append(solvent_graph)

        smiles_batch_graph = batch_graphs(smiles_graphs)
        solvent_batch_graph = batch_graphs(solvent_graphs)
        target = torch.from_numpy(np.concatenate([item[1] for item in items]))
        return (smiles_batch_graph, solvent_batch_graph), target


       


class SolventGraphDataLoader(torch.utils.data.DataLoader):
    ...
    **With Distributed Data Parallel**
    ...
    collator_arglist = inspect.getfullargspec(SolventGraphCollator).args
    ...
    def __init__(
        ...
        if collate_fn is None and kwargs.get("batch_size", 1) is not None:
            collate_fn = SolventGraphCollator(**collator_kwargs).collate
