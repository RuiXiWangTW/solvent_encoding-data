...
import dgl
from solvent_dataloader import SolventGraphDataLoader
...
class SolventMolGraphSet(Dataset):
    def __init__(self,df,target,log=print):
        ...
        for i,row in df.iterrows():
            ...
            try:
                ...
                log(e,'invalid',smi)
            try:
                solvent = Chem.MolFromSmiles(solvent_smi)
                if solvent is None:
                    log('invalid',solvent_smi)
                else:
                    g = Mol2HeteroGraph(solvent)
                    if g.num_nodes('a') == 0:
                        log('no edge in graph',solvent_smi)
                    else:
                        self.solvents.append(solvent)
                        self.solvent_graphs.append(g)
            except Exception as e:
                log(e,'invalid',solvent_smi)                
    def __len__(self):
        return len(self.mols)
    
    def __getitem__(self,idx):
        return (self.graphs[idx], self.solvent_graphs[idx]) ,self.labels[idx]
    
def create_dataloader(args,filename,shuffle=True,train=True):
    dataset = SolventMolGraphSet(pd.read_csv(os.path.join(args['path'],filename)),args['target_names'])
    if train:
        batch_size = args['batch_size']
    else:
        batch_size = min(4200,len(dataset))
    
    dataloader = SolventGraphDataLoader(dataset,batch_size=batch_size,shuffle=shuffle)
