...
from data_with_descriptors import create_dataloader
from descriptor_model import PharmHGT as Model
from schedular import NoamLR
from utils import get_func,remove_nan_label, mae, r2_score

def evaluate(dataloader,model,device,metric_fn,metric_dtype,task):
    metric = 0
    for bg,labels in dataloader:
        bg,labels = (bg[0].to(device), bg[1].to(device)),labels.type(metric_dtype)
        ...
        for fold in range(train_args['num_fold']):
            ...
            for epoch in tqdm(range(epochs)):
                ...
                for bg,labels in trainloader:
                    bg,labels = (bg[0].to(device), bg[1].to(device)),labels.type(loss_dtype).to(device)
                    pred = model(bg)
                    num_task =  pred.size(1)
                    if num_task > 1:
                        loss = 0
            ...
            mae_test_metric = evaluate(testloader,model,device,mae,metric_dtype,data_args['task'])
            print(f'\nMAE of test-set:{mae_test_metric}')

            r2_test_metric = evaluate(testloader, model, device, r2_score, metric_dtype, data_args['task'])
            print(f'\nr2 of test-set:{r2_test_metric}')
