...
import dgl
from descriptor_dataloader import DescriptorGraphDataLoader
...
class MolGraphSet(Dataset):
    def __init__(self,df,target, descriptor, log=print):
        self.data = df
        self.mols = []
        self.labels = []
        self.graphs = []
        ...
        if descriptor == "mn":
            descriptor_length = 7
        elif descriptor == "catalan":
            descriptor_length = 4
        elif descriptor == "reichardt":
            descriptor_length = 2
        else:
            raise NameError("Descriptor Not Supported")
        for i,row in df.iterrows():
            smi = row['smiles']
            label = row[target].values.astype(float)
            descriptor = torch.reshape(torch.tensor([row.iloc[i] for i in range(4, 4 + descriptor_length)]), (1, descriptor_length))
            try:
...
def create_dataloader(args,filename,shuffle=True,train=True):
    dataset = MolGraphSet(pd.read_csv(os.path.join(args['path'],filename)),args['target_names'], args["descriptor"])
    if train:
        batch_size = args['batch_size']
    else:
        batch_size = min(4200,len(dataset))
    
    dataloader = DescriptorGraphDataLoader(dataset,batch_size=batch_size,shuffle=shuffle)
